...
export class AddPartnerCpmModel1747400000 implements MigrationInterface {
  name = 'AddPartnerCpmModel1747400000'
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      DROP INDEX IF EXISTS "IDX_partner_cpm_visitors_partner_day";
    `)
    await queryRunner.query(`
      DROP INDEX IF EXISTS "IDX_partner_cpm_visitors_partner_hash_day_unique";
    `)
    await queryRunner.query(`
      DROP TABLE IF EXISTS "partner_cpm_visitors";
    `)
    await queryRunner.query(`
      DROP INDEX IF EXISTS "IDX_partner_cpm_daily_stats_partner_day";
    `)
    await queryRunner.query(`
      DROP INDEX IF EXISTS "IDX_partner_cpm_daily_stats_partner_code_day_unique";
    `)
    await queryRunner.query(`
      DROP TABLE IF EXISTS "partner_cpm_daily_stats";
    `)
    await queryRunner.query(`
      ALTER TABLE "partner_levels"
      DROP COLUMN IF EXISTS "cpm_rate";
    `)
  }
}
